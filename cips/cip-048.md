...
1. **Evidence parameter adjustment**: The `MaxAgeNumBlocks` parameter MUST be increased from 242,640 to 559,938 blocks so that it continues to represent approximately the same wall-clock duration (~16.8 days) at the ~2.6-second effective block time. The `MaxAgeDuration` parameter (337 hours) remains unchanged as it is defined in wall-clock time rather than block count.
...
1. **Upgrade height delay adjustments**: All upgrade height delay parameters MUST be scaled to maintain the same wall-clock durations at the ~2.6-second effective block time:
    ...
    1. `MainnetUpgradeHeightDelay` SHALL be increased from 100,800 to 232,615 blocks (~7 days at ~2.6 seconds per block).
...
| `consensus.evidence.MaxAgeNumBlocks` | 242,640 blocks | 559,938 blocks | Maximum number of blocks for which evidence can be submitted for slashing | No |
...
| `MainnetUpgradeHeightDelay` | 100,800 blocks | 232,615 blocks | Number of blocks Mainnet waits after a MsgTryUpgrade to activate the next version (~7 days) | No |
