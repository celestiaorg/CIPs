...
Prover programs can be written by networks using popular ZKVMs like SP1 and Risc0. This provides networks an option to upgrade to a secure, proof-based path to transfer assets via Celestia.
...
The `x/zkism` module implements a Hyperlane Interchain Security Module that maintains opaque state along with Groth16 verifier configuration. The first 32 bytes of state serve as a trusted state root used for membership verification.
...
2. **StateMembershipValues**: Contains the state root and a list of message IDs to authorize. The proof demonstrates that the specified messages are included in the state represented by the root.
...
|--------|--------|--------------|
| **On-chain cost** | Higher (Groth16 verification more expensive than simple signature verification, though depends on number of signatures) | Lower |
