...
| authors            | Hlib Kanunnikov ([@Wondertan](https://github.com/Wondertan))     |
|                    | Viacheslav Gonkivskyi ([@vgonkivs](https://github.com/vgonkivs)) |
...
NamespaceDataID identifies [namespace][ns] Data container of shares within a single [DataSquare][square]

NamespaceDataID identifiers are formatted as shown below:
...
The fields with validity rules that form NamespaceDataID are:

[**EdsID**](#edsid): A EdsID of the NamespaceData container. It MUST follow [EdsID](#edsid) formatting and field validity rules.

[**Namespace**][ns]: A fixed-size 29 bytes array representing the Namespace of interest. It MUST follow [Namespace][ns]
formatting and its validity rules.

[Serialized](#serialization-for-share-identifiers) NamespaceDataID MUST have a length of 37 bytes.

#### NamespaceData Container

NamespaceData containers encapsulate user-submitted data under [namespaces][ns] within an entire [DataSquare][square]. Namespace data may span over multiple rows.

NamespaceData containers are protobuf formatted using the following proto3 schema:
...
**NamespaceData**: A variable size [RowNamespaceData](#rownamespacedata-container) array representing data of a namespace in a [DataSquare][square].

#### RangeNamespaceDataID

RangeNamespaceDataID identifies a continuous range of shares under the same namespace within an ODS.

RangeNamespaceDataID identifiers formatted as shown below:
...
[**EdsID**](#edsid): A EdsID of the RangeNamespaceData container. It MUST follow [EdsID](#edsid) formatting and field validity rules.

**From**: A uint32 representing the first index of the sampled share within the ODS. The 32 bit
limit fits data squares up to 2TB. It MUST stay within the number of original shares(1st quadrant) of the EDS.

**To**: A uint32 representing the last index of the sampled share within the ODS(exclusively). The 32 bit
limit fits data squares up to 2TB. It MUST stay within the number of original shares(1st quadrant) of the EDS, increased by one.

#### RangeNamespaceData Container

RangeNamespaceData containers encapsulate user-submitted data under speicific coordinates [from;to) within a [DataSquare][square].

RangeNamespaceData containers are protobuf formatted using the following proto3 schema:
...
message RangeNamespaceData {
  ...
  Proof firstIncompleteRowProof = 2;
  Proof lastIncompleteRowProof = 3;
}
```

The fields with validity rules that form Data containers are:

**Shares**: A variable size [RowShares](#rowshares) array representing data of a range in a single [DataSquare][square].

**FirstIncompleteRowProof**: An [NMT][nmt] proof for the first row, present when:

- The range starts at a non-zero share index, OR
- In single-row ranges, when the range is not full (less than the number of shares in the row)

**LastIncompleteRowProof**: An [NMT][nmt] proof for the last row in multi-row ranges, present when the range ends before the row's final share.

Both Proofs can be empty.
...
RowShares defines an array of [shares][shares] within a single row of the [data square][square].
...
The fields with validity rules that form RowShares container are:

**Shares**: A variable size [Share](#share) array representing original shares of a row. Its length MUST be equal to the number of Column roots in [DAH][dah] divided by two.
...
|----------------------|-----------|--------|
| EdsID*               | N/A       | N/A    |
| NamespaceDataID*     | N/A       | N/A    |
| RowID                | 0x7801    | 0x7800 |
| SampleID             | 0x7811    | 0x7810 |
| RowNamespaceDataID   | 0x7821    | 0x7820 |
| RangeNamespaceDataID | 0x7831    | 0x7830 |
