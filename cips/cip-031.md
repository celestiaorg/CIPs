...
Accounts are able to set different withdraw addresses to their own in the creation of a delegation or after the fact, due to rewards needing to adhere to the original vesting schedule the withdraw address must be the same as the delegator address. This is to ensure that the rewards are locked up in the same account. Vesting accounts that have different withdraw addresses will have their withdraw address ignored. This means that the original vesting account will be used as the withdraw address.
