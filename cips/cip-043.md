...
Note: Inbound IBC and Hyperlane transfers cannot be rejected at the protocol level. Non-utia tokens sent via these bridges will become permanently stuck at the fee address.
...
1. **Denom Restriction**: Only `utia` can be sent to the fee address via direct sends (`MsgSend`/`MsgMultiSend`), enforced by the `FeeAddressDecorator` ante handler. Inbound IBC and Hyperlane transfers cannot be rejected at the protocol level—non-utia tokens sent via these bridges will become permanently stuck at the fee address with no way to recover them. Users and applications should ensure they only send `utia` to the fee address.
