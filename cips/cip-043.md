...
| description | Forward TIA tokens to validators via a dedicated fee address |
...
This proposal introduces a fee address module (`x/feeaddress`) that allows users to send TIA tokens to a dedicated address, which are then automatically forwarded to validators as transaction fees via protocol-injected transactions.

## Motivation

The fee address mechanism enables applications to direct value to validators in a way that is compatible with blockchain analytics dashboards. For example:

- Rollups could send a portion of blob fees to validators
- Applications could share protocol revenue with the validator set
- Cross-chain bridges (e.g., Hyperlane) could forward fees to validators
...
The distribution module allocates fee collector funds to validators at `BeginBlock`. This is existing Cosmos SDK functionality - no changes are required. The transaction fee from `MsgForwardFees` is distributed to validators as staking rewards according to the standard distribution logic.
...
```shell
# Send tokens to the fee address (forwarded to validators in next block)
