...
External networks using Celestia for data availability may want to contribute fee revenue to Celestia. However, they face operational constraints:

1. **No native Celestia account**: External networks typically don't maintain a funded account on Celestia with signing keys, and setting up a custodian/multisig on Celestia adds operational overhead
2. **Bridge-only access**: They can only send funds via cross-chain bridges (IBC, Hyperlane)
3. **Bridges can't set tx fees**: Cross-chain transfers deliver tokens to a recipient but cannot submit transactions with custom fee amounts

This creates a gap: networks can bridge TIA to Celestia, but cannot convert those tokens into Celestia protocol revenue without these operational constraints.

The fee address solves this by providing a dedicated address where:
- Networks can send TIA via any bridge (IBC, Hyperlane, etc.)
- Tokens are automatically converted to Celestia protocol revenue via protocol-injected transactions
- Contributions appear in dashboard revenue calculations (e.g., Blockworks, Celenium) without requiring additional integration overhead from their end

The protocol-injected transaction approach is necessary because dashboards track revenue by aggregating `tx.AuthInfo.Fee` fields. A simpler design using direct bank transfers wouldn't appear as fee revenue in these analytics tools.
...
5. **Spam Resistance**: The fee forward transaction is created and prepended by the block proposer in PrepareProposal, not sourced from the mempool. Users cannot prevent its inclusion by filling blocks with spam transactions, as the fee forward tx is added before any user transactions are processed. A malicious proposer could omit it, but ProcessProposal would reject their block, causing them to forfeit proposer rewards with no economic gain—tokens simply remain in the fee address until the next honest proposer forwards them.
